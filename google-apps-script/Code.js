...
function createMatch(payload) {
  ...
  if (payload._update) {
    const status = requireOneOf(payload.status, ['scheduled', 'locked', 'finished'], '상태');
  ...
  const match = {
    ...
    status: requireOneOf(payload.status || 'scheduled', ['scheduled', 'locked', 'finished'], '상태'),
    result: '',
    settled: false,
...
function submitPrediction(payload) {
  ...
  if (match.status !== 'scheduled') throw new Error('예측이 마감된 경기입니다.');
  ...
  if (duplicate) throw new Error('이미 이 경기에 예측을 제출했습니다.');

  const user = getOrCreateUser(userName);
  if (Number(user.points) < points) throw new Error('포인트가 부족합니다.');
...
function getMatchPool(payload) {
  ...
  readRows('predictions').forEach((prediction) => {
    if (prediction.match_id !== matchId || !CHOICES.includes(prediction.choice)) return;
...
function settleMatch(payload) {
  ...
  const match = findRow('matches', 'id', matchId);
  if (!match) throw new Error('경기를 찾을 수 없습니다.');
  if (toBoolean(match.settled)) throw new Error('이미 정산된 경기입니다.');
  if (!CHOICES.includes(match.result)) throw new Error('경기 결과를 먼저 입력하세요.');

  const predictions = readRows('predictions').filter((prediction) => prediction.match_id === matchId);
  const total = predictions.reduce((sum, prediction) => sum + (Number(prediction.points) || 0), 0);
  const winners = predictions.filter((prediction) => prediction.choice === match.result);
  const winnerTotal = winners.reduce((sum, prediction) => sum + (Number(prediction.points) || 0), 0);
  const payoutsByPredictionId = {};

  if (total > 0 && winnerTotal > 0) {
    winners.forEach((prediction) => {
      payoutsByPredictionId[prediction.id] = Math.floor((Number(prediction.points) / winnerTotal) * total);
    });
  }
  ...
  updateRow('matches', 'id', matchId, { settled: true, status: 'settled' });
  writeAudit('settleMatch', { matchId, winnersCount: winners.length });
...
function requireAdmin(adminPassword) {
  ...
  if (!expected) throw new Error('ADMIN_PASSWORD 스크립트 속성을 설정하세요.');
  if (String(adminPassword || '') !== expected) throw new Error('관리자 비밀번호가 올바르지 않습니다.');
}
...
function normalizeUserName(value) {
  ...
  if (userName.length > 30) throw new Error('사용자 이름은 30자 이하로 입력하세요.');
  return userName;
}
...
function updateRow(sheetName, key, value, updates) {
  const sheet = getSheet(sheetName);
  const found = readRowsWithIndex(sheetName).find(({ row }) => String(row[key]) === String(value));
  if (!found) throw new Error('수정할 행을 찾을 수 없습니다.');
